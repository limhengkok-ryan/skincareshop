...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const cartCount = context ? context.cart.reduce((sum, item) => sum + item.quantity, 0) : 0;

  return (
    <>
      <header className="fixed top-0 w-full z-50 glass-nav shadow-sm bg-white/80 backdrop-blur-md">
        <div className="flex justify-between items-center w-full px-margin-mobile md:px-margin-desktop py-4 max-w-container-max mx-auto">
          ...
          <nav className="hidden md:flex items-center gap-8">
            <Link className="text-secondary hover:text-primary transition-colors font-body-md text-body-md" to="/">Home</Link>
          ...
          <div className="flex items-center gap-4">
            <Link to="/cart" className="text-earth-text hover:opacity-80 transition-opacity relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-gutter mb-12">
            <div className="md:col-span-1">
              ...
              <div className="flex gap-4">
                <a className="text-earth-text hover:text-primary transition-colors" href="#">
